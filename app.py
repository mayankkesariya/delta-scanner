...
with filter_bar:
    c1, c2, c3, c4 = st.columns([1.2, 1.4, 1.2, 1])
...
if auto_refresh:
    st_autorefresh(
        ...
        limit=None,
        key=f"refresh_{refresh_seconds}"
    )
...
try:
    ...
    option_rows = fetch_option_chain(underlying, selected_expiry)
    chain = build_chain_table(option_rows)

    st.caption(
        f"Refresh: {'ON' if auto_refresh else 'OFF'} | "
        f"Every: {refresh_seconds} sec | "
        f"Rows: {len(option_rows)}"
    )

    if option_rows:
        raw_df = pd.DataFrame(option_rows)
        spot_candidates = pd.to_numeric(raw_df.get("spot_price"), errors="coerce").dropna()
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"delta_{underlying.lower()}_option_chain_{selected_expiry}.csv",
        mime="text/csv"
    )
